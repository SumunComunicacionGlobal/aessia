...
// add_action( 'pre_get_posts', 'draft_posts' );
function draft_posts( $query ) {

    if ( is_admin() ) return;
    if ( 1 != get_current_user_id() ) return;
...
function filter_content( $content ) { 
    ...
    if ('' == $content ) {
        // $content .= wpautop( '<h3>Lorem ipsum dolor sit amet, consectetur adipiscing elit</h3>' );
        $content .= wpautop( 'Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.' ); 
...
function thumbnail_id_placeholder( $value, $object_id, $meta_key, $single ) {
    ...
    if ( !$has_thumb && !is_admin() && '_thumbnail_id' == $meta_key ) {
        $placeholder_id = get_theme_mod( 'aessia-imagen-por-defecto', false );
        if ( $placeholder_id ) return $placeholder_id;
    }
    return $value;
}
